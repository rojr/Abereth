...
public class ABMapLoader {
    ...
    public static ABBlockMap generateMap(String image) throws SlickException {

        ABBlockMap returnValue;

        Image img = new Image(image);
        int width = img.getWidth();
        int height = img.getHeight();
        returnValue = new ABBlockMap(width, height);
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                if (type != null) {
                    ABBlock tile = getBlockType(x, y, type);
    ...
    private static ABBlock getBlockType(int x, int y, BLOCK_TYPE type) {
        switch (type) {
            case WATER:
